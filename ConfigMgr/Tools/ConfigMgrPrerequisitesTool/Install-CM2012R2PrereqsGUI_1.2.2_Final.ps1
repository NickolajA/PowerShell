...
function Extend-ActiveDirectorySchema {
	...
	if (($WindowsPrincipal.IsInRole("Domain Admins")) -and ($WindowsPrincipal.IsInRole("Schema Admins"))) {
    	...
    	$Destination = "\\" + $DC + "\C$"
		Write-OutputBox -OutputBoxMessage "Copying EXTADSCH.EXE to the specified domain controller" -Type "INFO: "
    	Copy-Item $Path $Destination -Force
		Write-OutputBox -OutputBoxMessage "Starting to extend the Active Directory schema" -Type "INFO: "
    	...
    	if ($Content -match "Successfully extended the Active Directory schema") {
        	Write-OutputBox -OutputBoxMessage "Active Directory schema was successfully extended" -Type "INFO: "
...
$Form.Text = "ConfigMgr 2012 R2 Prerequisites Installation Tool - 1.2.2"
...
$GBSystemManagementContainerGroupSearch.Text = "Search for AD group to add to the System Management Container"
