<#
.SYNOPSIS
    Disable or Enable Windows Store updates during reference image creation using MDT.

.DESCRIPTION
    This script disables or enables Windows Store updates to occur during a reference image creation operation with MDT.
    In the Post Install phase, running in WinPE, run the script with the parameter State set for Disable. At the end of
    the task sequence under the Capture Image group (before Apply Windows PE), run the script with the parameter State set for Enable.
...
.EXAMPLE
    ...
    .\Set-WindowsStoreUpdates.ps1 -State Disable

    # Enable Windows Store updates:
    .\Set-WindowsStoreUpdates.ps1 -State Enable

.NOTES
    FileName:    Set-WindowsStoreUpdates.ps1
