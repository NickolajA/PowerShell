...
try {
    ...
    if (-not(Test-Path -Path $NTFSSecurityModulePath -PathType Container)) {
        Write-Output "Staging NTFSSecurity module in: $($NTFSSecurityModulePath)"
    ...
    Write-Warning -Message "Unable to stage required PowerShell module: NTFSSecurity" ; break
}
...
try {
    ...
    Write-Warning -Message "Unable to import required PowerShell module: NTFSSecurity" ; break
}
...
try {
    ...
    if ($CurrentOwner.Owner -notlike $NewOwner) {
        Write-Output "Setting new owner of '$($NewOwner)' on: $($DefaultWallpaperImagePath)"
    ...
    try {
        Write-Output "Granting '$($SystemContext)' Full Control on: $($DefaultWallpaperImagePath)"
        ...
        Write-Output "Granting '$($NewOwner)' Full Control on: $($DefaultWallpaperImagePath)"
        ...
        Write-Warning -Message "Unable to grant required Full Control permissions on: $($DefaultWallpaperImagePath)" ; break
    }

    # Replace wallpaper
    try {
        Write-Output "Replacing default wallpaper in: $($DefaultWallpaperImagePath)"
        ...
        Write-Warning -Message "Unable to replace default wallpaper: $($DefaultWallpaperImagePath)" ; break
    }
}
catch [System.Exception] {
